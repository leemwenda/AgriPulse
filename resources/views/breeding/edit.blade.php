    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('breeding.update', $breeding) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="animal_id" id="animal_id" required
                                ...
                                @foreach($animals as $animal)
                                    <option value="{{ $animal->id }}" {{ old('animal_id', $breeding->animal_id) == $animal->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="service_date" id="service_date" 
                                   value="{{ old('service_date', $breeding->service_date->format('Y-m-d')) }}" required
                                   class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500">
                            @error('service_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="bull_name" id="bull_name" value="{{ old('bull_name', $breeding->bull_name) }}"
                                   class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500">
                            @error('bull_name')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="expected_birth_date" id="expected_birth_date" 
                                   value="{{ old('expected_birth_date', $breeding->expected_birth_date ? $breeding->expected_birth_date->format('Y-m-d') : '') }}"
                                   class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500">
                        ...
                        <!-- Actual Birth Date -->
                        <div class="mb-4">
                            <label for="actual_birth_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                Actual Birth Date
                            </label>
                            <input type="date" name="actual_birth_date" id="actual_birth_date" 
                                   value="{{ old('actual_birth_date', $breeding->actual_birth_date ? $breeding->actual_birth_date->format('Y-m-d') : '') }}"
                                   class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="pregnancy_status" id="pregnancy_status" required
                                    class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500">
                                <option value="pending" {{ old('pregnancy_status', $breeding->pregnancy_status) == 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="pregnant" {{ old('pregnancy_status', $breeding->pregnancy_status) == 'pregnant' ? 'selected' : '' }}>Pregnant</option>
                                <option value="gave_birth" {{ old('pregnancy_status', $breeding->pregnancy_status) == 'gave_birth' ? 'selected' : '' }}>Gave Birth</option>
                                <option value="failed" {{ old('pregnancy_status', $breeding->pregnancy_status) == 'failed' ? 'selected' : '' }}>Failed</option>
                            </select>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="notes" id="notes" rows="3"
                                      class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500">{{ old('notes', $breeding->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
