    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Edit Milk Production Record') }}
            </h2>
                    ...
                    <form method="POST" action="{{ route('milk-production.update', $milkProduction) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="animal_id" id="animal_id" required
                                ...
                                @foreach($animals as $animal)
                                    <option value="{{ $animal->id }}" {{ old('animal_id', $milkProduction->animal_id) == $animal->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="production_date" id="production_date" 
                                   value="{{ old('production_date', $milkProduction->production_date->format('Y-m-d')) }}" required
                                   class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="quantity_liters" id="quantity_liters" 
                                   value="{{ old('quantity_liters', $milkProduction->quantity_liters) }}" required
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="notes" id="notes" rows="3"
                                      class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500">{{ old('notes', $milkProduction->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
