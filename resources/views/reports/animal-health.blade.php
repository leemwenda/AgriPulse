        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <p class="text-sm text-gray-500 dark:text-gray-400">Total Health Records</p>
                    <p class="text-2xl font-bold text-blue-600">{{ $totalRecords ?? 0 }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <p class="text-sm text-gray-500 dark:text-gray-400">This Month</p>
                    <p class="text-2xl font-bold text-green-600">{{ $monthRecords ?? 0 }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <p class="text-sm text-gray-500 dark:text-gray-400">Vaccinations</p>
                    <p class="text-2xl font-bold text-purple-600">{{ $vaccinationCount ?? 0 }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <p class="text-sm text-gray-500 dark:text-gray-400">Treatments</p>
                    <p class="text-2xl font-bold text-orange-600">{{ $treatmentCount ?? 0 }}</p>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    @else
                    <p class="text-gray-500 dark:text-gray-400">No health records found.</p>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Common Conditions</h3>
                    @if(isset($commonConditions) && $commonConditions->count() > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach($commonConditions as $condition)
                        <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                            <p class="font-medium text-gray-900 dark:text-gray-100">{{ $condition->condition }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $condition->count }} occurrences</p>
                        </div>
                        @endforeach
                    </div>
                    @else
                    <p class="text-gray-500 dark:text-gray-400">No condition data available.</p>
                    @endif
                </div>
            </div>
        </div>
